...
function debugout() {
	...
	this.setFileName = function(name){
		self.fileName = name;
		self.logFilename = name+self.fileExtension;
	}
	...
	*/	
	this.getLog = function() {
		...
		if (!self.recordLogs) {
			self.log('['+self.fileName +'.js] log recording is off.');
		}
		// if using local storage, get values
		if (self.useLocalStorage) {
			var saved = window.localStorage.getItem(self.fileName+'.js');
			if (saved) {
	...
	this.getLogByName = function(fileName) {
		var retrievalTime = new Date();
		let savedData;
		// if recording is off, so dev knows why they don't have any logs
		if (!self.recordLogs) {
			self.log('['+fileName +'.js] log recording is off.');
		}
		// if using local storage, get values
		if (self.useLocalStorage) {
			var saved = window.localStorage.getItem(fileName+'.js');
			if (saved) {
				saved = JSON.parse(saved);
				self.startTime = new Date(saved.startTime);
				savedData = saved.log;
			...
			self.clearLogsByFileName(fileName+'js');
		}
		return savedData
			+ '\n---- Log retrieved: '+retrievalTime+' ----\n'
			+ self.formatSessionDuration(self.startTime, retrievalTime);
	...
	this.downloadLogByFileName = function(fileName) {
	    var logFile = self.getLogByName(fileName);
            var blob = new Blob([logFile], { type: 'data:text/plain;charset=utf-8' });
            var a = document.createElement('a');
            a.href = window.URL.createObjectURL(blob);
            a.target = '_blank';
            a.download = fileName+self.fileExtension;
	...
	this.clear = function() {
		...
		if (self.useLocalStorage) {
			...
			window.localStorage.setItem(self.fileName+'.js', saveObject);
		}
		if (self.realTimeLoggingOn) console.log('['+self.fileName+ '.js] clear()');
	}

	this.clearLogsByFileName = function(name){
		window.localStorage.removeItem(name+'.js');
		console.log(name+'.js removed from local storage');
	}
